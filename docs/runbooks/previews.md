...
Each build request carries a build id, stored on the preview row, which the runner sends back with every report; a report for any other build, such as one a rebuild replaced, is ignored, and a runner that finds its build superseded after starting the container removes it. Reports are retried for about a minute, as a Session's exit is. `sha` is the commit being served and changes only when a build reaches running; a failed build's commit is kept as `failed_sha`. A rebuild does not take a working Preview away: the router keeps proxying to the previous container until the new one replaces it, and shows the holding page only when there is nothing to serve, or the error page when the build failed. A failed rebuild leaves the previous container running, but the router shows the failure.

A build lives only in the runner's memory, so a runner restart, which every deploy causes, loses the builds in flight. When the runner starts it tells the app when, and which build ids it has accepted since; every other Preview still building from before then is settled. A lost first build is marked failed, with an error saying the build was interrupted and the preview should be requested again. A lost rebuild of a Preview that still has a container goes back to running on it, with a note that the rebuild was interrupted. As a backstop the app settles any Preview still building five minutes past the build limit the same way, checked at boot and every five minutes. The app reads the limit from the same `KARDBOARD_PREVIEW_BUILD_TIMEOUT_MINUTES`, so if it is ever changed, set it on both the app and the runner.
