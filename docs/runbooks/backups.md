...
**When a Board was deleted by mistake**, the live database's `board.deleted` event names the snapshot taken just before; read it before restoring anything, from `~/docker/data/kardboard/app`: `sqlite3 kardboard.db "select payload from events where type = 'board.deleted'"`. Restore that snapshot as above, then copy the copy directory's `uploads/` back as for a lost disk, with `cp -an` so nothing already there is overwritten: the Board's attachments were removed from the data disk. Everything else done since the snapshot is lost with it, so restore only when the Board matters more than what changed since.
