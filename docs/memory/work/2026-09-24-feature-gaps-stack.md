...
- On minicore, `mkdir -p /nas/backup/minicore/kardboard && touch /nas/backup/minicore/kardboard/.kardboard-backup-target`, check uid 1000 can write there, then copy the `deploy/compose.yaml` changes (the `/nas/backup` bind with `rslave` propagation, `KARDBOARD_BACKUP_COPY_DIR`, the four health checks) into `chriscorbell/stacks` and `docker compose up -d`. Then check that `docker compose up -d --force-recreate app` still starts the app while the NAS is unreachable.
