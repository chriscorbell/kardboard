...
- ~~On minicore, create the backup directory and marker, copy the compose changes into `chriscorbell/stacks`, and check the app starts with the NAS unreachable.~~ Done 2026-09-24: `/nas/backup/minicore/kardboard/.kardboard-backup-target` exists. A throwaway container showed that binding an automount point whose server is unreachable fails the start ("no such device"), so the compose file binds `/nas` with `rslave` instead, which starts either way and mounts the share on demand from inside the container. The stacks compose carries the bind, `KARDBOARD_BACKUP_COPY_DIR`, and the four health checks.
