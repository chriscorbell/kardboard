...
- [The Codex CLI drops flags and config keys between releases](codex-cli-drifts-between-releases.md): read when a Codex Session exits at once or reaches no provider, when the egress log shows `refused codex` or `refused claude`, when editing the Codex branch of the entrypoint, or before bumping `CODEX_VERSION`.
