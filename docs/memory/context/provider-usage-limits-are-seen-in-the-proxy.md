...
- Since the feature-gap stack, `/limits` also reports, per Provider, a credential it rejected (401 or 403 on a turn call), refused request paths, and which credentials are loaded. The app polls it every five minutes and emails the Admin; only a 429 drives fallback. A malformed extra field never drops the limits themselves.
