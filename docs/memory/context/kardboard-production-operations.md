...
- Service health: `curl http://127.0.0.1:3070/healthz` on minicore, `docker compose ps` in the stack directory. The app's answer is 503 when it cannot read its database, and carries the runner's and egress proxy's last-seen state without failing on them; every service has a compose health check. The app also writes its console to `~/docker/data/kardboard/app/logs/app-YYYY-MM-DD.log`, kept 14 days, so a recreated container no longer takes its log with it. Public check: `https://kardboard.cc/healthz`. The current hostnames, Clerk, and email configuration are in [domain configuration](../../runbooks/domains.md).
